...
def listInvoiceHandler(event, context):
  ...
  content += ' <td><input type="radio" name="id" value="{{ x.invoice_id }}">{{ x.invoice_id}}</td><td>{{ x.email}}</td><td>{{ x.invoice_status }}</td><td>{{ x.item }}</td><td>{{ x.amount | currency }}</td><td>{{ x.invoice_date }}</td>'
  content += '<td>{{ x.payment_amount | currency }}</td><td>{{ x.payment_date }}</td><td>{{ x.payment_type }}</td>'
  content += '</tr>'
...
#    content += '</tr>'
  ...
  content += 'CANCELLED <input onclick="document.getElementById(\'listInvoices\').submit()" type="checkbox" name="filter" value="CANCELLED"'
  if 'CANCELLED' in filters:
    content += ' checked'
  content += '> '
  content += 'SENT <input onclick="document.getElementById(\'listInvoices\').submit()" type="checkbox" name="filter" value="SENT"'
  if 'SENT' in filters:
    content += ' checked'
  content += '> '
  content += 'PAID <input onclick="document.getElementById(\'listInvoices\').submit()" type="checkbox" name="filter" value="PAID"'
  if 'PAID' in filters:
    content += ' checked'
  content += '></td>'
  content += "</tr></table>"
